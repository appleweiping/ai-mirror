...
11 以上の主要・オープンモデルと誰でもチャットでき、瞬時に切り替えられるシンプルな Web UI。
...
- **11 プロバイダ、1 UI** — ほぼすべてが OpenAI 互換プロトコル。Claude の `/v1/messages`
  は透過的に変換され、ブラウザは 1 つの形式だけを扱います。
- **モデル選択と上書き** — リストから選択、または Doubao / Hugging Face / リレー用の実モデル・エンドポイント ID を入力できます。
- **改善されたチャット操作** — クイックプロンプト、生成停止、回答コピー、プロバイダ別の会話クリア、Key 不足時の明確な案内。
...
静的 UI と `/api/*` を同時に動かすため、Vercel CLI で起動してください：

```bash
npm run dev
```

`http://localhost:3000` を開きます。`npm run dev:static` は見た目の確認専用で、API がないためチャットは無効です。
...
public/            静的フロントエンド（フレームワーク・ビルド不要）
  index.html       シェル
  app.js           状態、SSE ストリーミング、テーマ/モデル/Key 操作
  styles.css       レイアウト・コンポーネント（CSS 変数駆動）
  themes.css       11 ブランドテーマ × ライト/ダーク
  i18n.js          中 / 英 / 日 / 韓 文字列
  assets/          生成 banner と静的ビジュアル資産
api/               Vercel エッジ関数
