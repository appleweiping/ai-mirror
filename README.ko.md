...
11개 이상의 주요/오픈 모델과 누구나 대화하고 즉시 전환할 수 있는 깔끔한 웹 UI. 모델을 고르면
...
- **11개 제공업체, 1개 UI** — 거의 모두 OpenAI 호환 프로토콜. Claude 의 `/v1/messages`
  는 투명하게 변환되어 브라우저는 한 가지 형식만 다룹니다.
- **모델 선택과 재정의** — 목록에서 고르거나 Doubao / Hugging Face / relay 에 필요한 실제 모델·엔드포인트 ID 를 입력할 수 있습니다.
- **더 나은 채팅 조작** — 빠른 프롬프트, 생성 중지, 답변 복사, 제공업체별 대화 지우기, Key 부족 시 명확한 안내.
...
정적 UI 와 `/api/*` 를 함께 실행하려면 Vercel CLI 로 시작하세요:

```bash
npm run dev
```

`http://localhost:3000` 을 엽니다. `npm run dev:static` 은 시각 미리보기 전용이며 API 가 없어 채팅은 비활성화됩니다.
...
public/            정적 프론트엔드(프레임워크·빌드 없음)
  index.html       셸
  app.js           상태, SSE 스트리밍, 테마/모델/Key 조작
  styles.css       레이아웃·컴포넌트(CSS 변수 기반)
  themes.css       11개 브랜드 테마 × 라이트/다크
  i18n.js          중 / 영 / 일 / 한 문자열
  assets/          생성 banner 및 정적 비주얼 자산
api/               Vercel 엣지 함수
